classdef ImageReader
    ...
    methods
        function ir = ImageReader(src, L, R, varargin)%function with same name, to get parameters and update properties     
            ...
            p.addOptional('N',ir.N, @(x) isnumeric(x));%these 2 are optional, we have default value for them
            ...
            ir.start=max(ir.start,1);%0 means index 1 for the images, so set start min 1
            ir.N = p.Results.N;%number of following images
            ir.allimage_left=dir(strcat(ir.l,"*.jpg"));%read all image informations for next()
        ...
        function [left, right, loop] = next(ir)%function to read image further and further
            ...
            if index+ir.N<numel(ir.allimage_left)%if not reached the end
                for i=1:ir.N+1%read N+1 images and pack them together to left and right
            ...
            else%reached the end, set loop, and set start to begin
                num_rest=numel(ir.allimage_left)-index+1;%number of the rest images
